...
{
	public Event13(Handler handler)
	{
		super(handler);
	...
	{
		if(handler.getGame().getGameState().getLevelManager().getBaseCamp().getGameProgress() == 15 && handler.getGame().getGameState().getLevelManager().getBaseCamp().getState() == "MAINAREA")
		{
	...
	public void run(Graphics g) 
	{
		...
		case 1:
			...
			{
				handler.getGame().getGameState().getEffects().displayDialog(103);
				delay(500);
			}
			break;
		case 2:
			if(!handler.getGame().getGameState().getEffects().isActiveDBox() && handler.getCamera().scrawl(player.getX() - handler.getGame().getWidth() / 2 + player.getWidth() / 2, player.getY() - handler.getGame().getHeight() / 2 + player.getHeight() / 2))
			{
				...
				handler.getGame().getGameState().getLevelManager().getBaseCamp().setGameProgress(16);
				handler.getBaseCamp().setDungeonsUnlocked(5);
				running = false;
			}
			break;
		}
