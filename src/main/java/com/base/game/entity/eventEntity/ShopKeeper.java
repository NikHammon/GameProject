...
public class ShopKeeper {
		...
		if (state == "CHOOSEOP") {
			if (index >= text.length()) {
				...
				if (selector == 0)
					...
					selectorBox.setAll(
							new Rectangle(handler.getWidth() - 205, handler.getGame().getHeight() - 505, 200, 100));
			...
			if (selectorLocation > Math.max(0, Math.min(
					handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().size() - 1, 14)))
				selectorLocation = Math.max(0, Math.min(
						handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().size() - 1,
						14));

			if (listSelector > Math.max(0,
					handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().size() - 1))
				listSelector = Math.max(0,
						handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().size() - 1);
			...
			if (handler.getInput().a) {
				if (shopItems.size() > 0) {
					if (handler.getGame().getPlayer().takeMoney(shopItems.get(listSelector).getValue())) {
						handler.getGame().getGameState().getLevelManager().getiInterface().getInventory()
								.add(shopItems.get(listSelector));
			...
			selectorBox.setAll(new Rectangle(handler.getWidth() / 2 + 22 + shiftInvX,
					142 + shiftInvY + selectorLocation * 20, 356, 20));
			...
			if (handler.getInput().downPressed) {
				if (listSelector < handler.getGame().getGameState().getLevelManager().getiInterface().getInventory()
						.size() - 1) {
					listSelector++;
					selectorLocation++;
				}
				if (listSelector >= bottom && bottom < handler.getGame().getGameState().getLevelManager()
						.getiInterface().getInventory().size()) {
					top++;
					bottom++;
				}
				if (selectorLocation > Math.max(0, Math.min(
						...
						14)))
					selectorLocation = Math.max(0, Math.min(
							handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().size()
									- 1,
							14));
			}
			if (handler.getInput().a) {
				if (handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().size() > 0) {
					handler.getGame().getPlayer().addMoney(handler.getGame().getGameState().getLevelManager()
							.getiInterface().getInventory().get(listSelector).getValue() / 5);
					handler.getGame().getGameState().getLevelManager().getiInterface().getInventory()
							.remove(listSelector);
				}
				if (listSelector > handler.getGame().getGameState().getLevelManager().getiInterface().getInventory()
	...
	public void render(Graphics g) {
		if (state == "CHOOSEOP") {
			...
			if (index >= text.length()) {
				...
				g.drawString("BUY", handler.getWidth() - 100 - g.getFontMetrics().stringWidth("BUY") / 2,
						handler.getGame().getHeight() - 440);
				g.drawString("SELL", handler.getWidth() - 100 - g.getFontMetrics().stringWidth("SELL") / 2,
						handler.getGame().getHeight() - 340);
				g.drawString("CANCEL", handler.getWidth() - 100 - g.getFontMetrics().stringWidth("CANCEL") / 2,
						handler.getGame().getHeight() - 240);
			...
			if (!handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().isEmpty()) {
				for (int i = top; i < Math.min(
					...
					if (i == selectorLocation) {
						...
						g.drawString(
								handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().get(i)
										.getName(),
								handler.getWidth() / 2 + 27 + shiftInvX, 159 + (i - top) * 20 + shiftInvY);
						g.drawString(
								Integer.toString(handler.getGame().getGameState().getLevelManager().getiInterface()
										.getInventory().get(i).getValue() / 5),
								handler.getWidth() / 2 + 347 + shiftInvX, 159 + (i - top) * 20 + shiftInvY);
						g.setColor(Color.WHITE);
						g.drawString(
								handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().get(i)
										.getName(),
								handler.getWidth() / 2 + 25 + shiftInvX, 157 + (i - top) * 20 + shiftInvY);
						g.setColor(new Color(90, 255, 140));
						g.drawString(
								Integer.toString(handler.getGame().getGameState().getLevelManager().getiInterface()
										.getInventory().get(i).getValue() / 5),
								handler.getWidth() / 2 + 345 + shiftInvX, 157 + (i - top) * 20 + shiftInvY);
						...
						g.drawString(
								handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().get(i)
										.getName(),
								handler.getWidth() / 2 + 32 + shiftInvX, 159 + (i - top) * 20 + shiftInvY);
						g.drawString(
								Integer.toString(handler.getGame().getGameState().getLevelManager().getiInterface()
										.getInventory().get(i).getValue() / 5),
								handler.getWidth() / 2 + 347 + shiftInvX, 159 + (i - top) * 20 + shiftInvY);
						...
						g.drawString(
								handler.getGame().getGameState().getLevelManager().getiInterface().getInventory().get(i)
										.getName(),
								handler.getWidth() / 2 + 30 + shiftInvX, 157 + (i - top) * 20 + shiftInvY);
						g.setColor(new Color(90, 255, 140));
						g.drawString(
								Integer.toString(handler.getGame().getGameState().getLevelManager().getiInterface()
										.getInventory().get(i).getValue() / 5),
				...
				g.drawString(handler.getGame().getGameState().getLevelManager().getiInterface().getInventory()
						.get(listSelector).getSummary()[0], 195, handler.getHeight() - 75);
				g.drawString(handler.getGame().getGameState().getLevelManager().getiInterface().getInventory()
						.get(listSelector).getSummary()[1], 195, handler.getHeight() - 60);
				g.drawString(handler.getGame().getGameState().getLevelManager().getiInterface().getInventory()
						.get(listSelector).getSummary()[2], 195, handler.getHeight() - 45);
			...
			if (!shopItems.isEmpty()) {
				for (int i = top; i < Math.min(shopItems.size(), bottom); i++) {
					if (i == selectorLocation) {
						g.setColor(Color.BLACK);
						g.setFont(new Font(handler.getGame().font, Font.BOLD, 20));
						g.drawString(shopItems.get(i).getName(), handler.getWidth() / 2 + 27 + shiftInvX,
								159 + (i - top) * 20 + shiftInvY);
						g.drawString(Integer.toString(shopItems.get(i).getValue()),
								handler.getWidth() / 2 + 347 + shiftInvX, 159 + (i - top) * 20 + shiftInvY);
						g.setColor((Integer.parseInt(handler.getGame().getPlayer().getMoney()) >= shopItems.get(i)
								.getValue()) ? Color.WHITE : new Color(240, 90, 110));
						g.drawString(shopItems.get(i).getName(), handler.getWidth() / 2 + 25 + shiftInvX,
								157 + (i - top) * 20 + shiftInvY);
						g.setColor((Integer.parseInt(handler.getGame().getPlayer().getMoney()) >= shopItems.get(i)
								.getValue()) ? new Color(90, 255, 140) : new Color(240, 90, 110));
						g.drawString(Integer.toString(shopItems.get(i).getValue()),
								handler.getWidth() / 2 + 345 + shiftInvX, 157 + (i - top) * 20 + shiftInvY);
						...
						g.setColor(Color.BLACK);
						g.setFont(new Font(handler.getGame().font, Font.BOLD, 15));
						g.drawString(shopItems.get(i).getName(), handler.getWidth() / 2 + 32 + shiftInvX,
								159 + (i - top) * 20 + shiftInvY);
						g.drawString(Integer.toString(shopItems.get(i).getValue()),
								handler.getWidth() / 2 + 347 + shiftInvX, 159 + (i - top) * 20 + shiftInvY);
						g.setColor((Integer.parseInt(handler.getGame().getPlayer().getMoney()) >= shopItems.get(i)
								.getValue()) ? Color.WHITE : new Color(240, 90, 110));
						g.drawString(shopItems.get(i).getName(), handler.getWidth() / 2 + 30 + shiftInvX,
								157 + (i - top) * 20 + shiftInvY);
						g.setColor((Integer.parseInt(handler.getGame().getPlayer().getMoney()) >= shopItems.get(i)
								.getValue()) ? new Color(90, 255, 140) : new Color(240, 90, 110));
						g.drawString(Integer.toString(shopItems.get(i).getValue()),
	...
	public void generateItems() {
		int numOfItems = ThreadLocalRandom.current().nextInt(4) + 6;
