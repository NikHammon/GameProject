...
public abstract class Creature extends Entity {
	...
	protected int stunThreshold;// how many update ticks to update right before a stun is applied (so creature
								// can go flying back)
	...
	public void move() {
		...
		if (!getCollidedWith(xMove, 0f).isPresent())
			moveX();
		else
			collided = true;
		if (!getCollidedWith(0f, yMove).isPresent())
			moveY();
		else
			collided = true;
	...
	public void setFalling() {
		...
		if (handler.getLevel()
				.getTile((int) lastPosX / Tile.TILE_WIDTH, (int) (lastPosY + 20) / Tile.TILE_HEIGHT, lastLayer)
				.isNotSolidGround()) {
			if (xMove < 0)
	...
	public void setStunDuration(int newStunDuration) {
		stunDuration = newStunDuration;
	}

	public void setStunThreshold(int stunThreshold) {
	...
	public boolean isStoppedByEnemy() {
		return collided;
	}

	public void setStoppedByEnemy(boolean stoppedByEnemy) {
		this.collided = stoppedByEnemy;
	}
	...
	public boolean isHoldLastPos() {
		return holdLastPos;
	}

	public void setHoldLastPos(boolean holdLastPos) {
		this.holdLastPos = holdLastPos;
	}
