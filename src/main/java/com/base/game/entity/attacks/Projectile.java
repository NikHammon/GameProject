...
public abstract class Projectile extends Attack
{
	...
	public static final int DEFAULT_PROJECTILE_WIDTH = 64,
							DEFAULT_PROJECTILE_HEIGHT = 64;
	...
	public Projectile(Handler handler, Entity user, float x, float y, int layer, int width, int height, float xDir, float yDir) 
	{
		super(handler, user, x, y, layer, width, height);
	...
	{	
		...
		if(doesNotCollideWithEntity || getCollidedWith(xMove, 0f) == null || getCollidedWith(xMove, 0f) == user || getCollidedWith(xMove, 0f) instanceof Attack && ((Attack)getCollidedWith(xMove, 0f)).getUser() == user)
			moveX();
		else
			effect(getCollidedWith(xMove, 0f));

		if(doesNotCollideWithEntity || getCollidedWith(0f, yMove) == null || getCollidedWith(0f, yMove) == user || getCollidedWith(0f, yMove) instanceof Attack && ((Attack)getCollidedWith(0f, yMove)).getUser() == user)
			moveY();	
		else
			effect(getCollidedWith(0f, yMove));
	}
