...
public class BaseCamp {
	...
	private Rectangle specialDoor, questBoard, shopKeeper, blackSmith, storage, bed, mainAreaToConnector,
			castleAreaToConnector, connectorToCastleArea, connectorToMainArea, bedroomToMainArea;
	...
	private int boatX;
	private boolean storing, sleeping;
	...
	public void update() {
		if (state == "MAINAREA" && !isEventRunning) { // try with event running
			if (handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(specialDoor)
					&& handler.getGame().getPlayer().getState() != "FORCEMOVE" && gameProgress > 2) {
				state = "TOSAIL";
				handler.getGame().getGameState().getEffects().displayDialog(20);
				handler.getGame().getPlayer().setPaused(true);

			} else if (handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(questBoard)
					&& handler.getInput().a
					&& (handler.getGame().getPlayer().getDirection() == 0
							|| handler.getGame().getPlayer().getDirection() == 7
							|| handler.getGame().getPlayer().getDirection() == 1)) {
			...
			} else if (activeShopKeeper && handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(shopKeeper)
					&& handler.getInput().a
					&& (handler.getGame().getPlayer().getDirection() == 0
							|| handler.getGame().getPlayer().getDirection() == 7
							|| handler.getGame().getPlayer().getDirection() == 1)) {
				state = "SHOPKEEPER";
				handler.getGame().getPlayer().setPaused(true);
			} else if (activebSmith && handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(blackSmith)
					&& handler.getInput().a
					&& (handler.getGame().getPlayer().getDirection() == 0
							|| handler.getGame().getPlayer().getDirection() == 7
							|| handler.getGame().getPlayer().getDirection() == 1)) {
				state = "BLACKSMITH";
				handler.getGame().getPlayer().setPaused(true);
			} else if (nextArea != "CONNECTOR"
					&& handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(mainAreaToConnector)
					&& handler.getGame().getPlayer().getState() != "FORCEMOVE") {
			...
			if (nextArea != "MAINAREA"
					&& handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(connectorToMainArea)
					&& handler.getGame().getPlayer().getState() != "FORCEMOVE") {
				handler.getGame().getGameState().getEffects().fade(1);
				...
				setBarriers();
				nextArea = "";
			} else if (nextArea != "CASTLEAREA"
					&& handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(connectorToCastleArea)
					&& handler.getGame().getPlayer().getState() != "FORCEMOVE") {
				handler.getGame().getGameState().getEffects().fade(1);
			...
			if (nextArea != "CONNECTOR"
					&& handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(castleAreaToConnector)
					&& handler.getGame().getPlayer().getState() != "FORCEMOVE") {
				handler.getGame().getGameState().getEffects().fade(1);
			...
			if (handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(storage) && handler.getInput().a
					&& (handler.getGame().getPlayer().getDirection() == 5
							|| handler.getGame().getPlayer().getDirection() == 6
							|| handler.getGame().getPlayer().getDirection() == 7)) {
				storing = true;
				handler.getGame().getGameState().getLevelManager().getpInterface().setActive(false);
				handler.getGame().getPlayer().setPaused(true);
			...
			if (nextArea != "MAINAREA"
					&& handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(bedroomToMainArea)
					&& handler.getGame().getPlayer().getState() != "FORCEMOVE") {
				handler.getGame().getGameState().getEffects().fade(1);
				handler.getGame().getPlayer().setForceMove("DOWN", "DOWN");
				nextArea = "MAINAREA";
	...
	public void setNPCs() {
		...
		handler.getGame().geteManager().getEntities().removeIf((Entity e) -> e instanceof NPC);
	...
	public void setBarriers() {
		handler.getGame().geteManager().getEntities().removeIf((Entity e) -> (e instanceof Barrier));
		...
		if (state == "CONNECTOR") {
			...
			// if(gameProgress < 3)
			// handler.getGame().geteManager().addEntity(new Barrier(handler, 574, 1620, 0,
			// 68, 108));
	...
	public void render(Graphics g) {
		if (state == "CASTLEAREA") {
			currentWidth = Assets.townCastle.getWidth() * 2;
			currentHeight = Assets.townCastle.getHeight() * 2;
			g.drawImage(Assets.townCastle, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), Assets.townCastle.getWidth() * 2,
					Assets.townCastle.getHeight() * 2, null);

			for (Entity e : handler.getGame().geteManager().getEntities())
				handler.getGame().geteManager().render(e, g);

			g.drawImage(Assets.townCastleTop, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), Assets.townCastle.getWidth() * 2,
					Assets.townCastle.getHeight() * 2, null);

		} else if (state == "CONNECTOR") {
			currentWidth = Assets.connection1.getWidth() * 2;
			currentHeight = Assets.connection1.getHeight() * 2;
			g.drawImage(Assets.connection1, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), Assets.connection1.getWidth() * 2,
					Assets.connection1.getHeight() * 2, null);

			for (Entity e : handler.getGame().geteManager().getEntities())
				handler.getGame().geteManager().render(e, g);
			g.drawImage(Assets.connectionTop, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), Assets.connectionTop.getWidth() * 2,
					Assets.connectionTop.getHeight() * 2, null);

		} else if (state == "BEDROOM" || state == "TOSAVE" || state == "SAVING") {
			currentWidth = Assets.bedroom.getWidth() * 2;
			currentHeight = Assets.bedroom.getHeight() * 2;
			g.drawImage(Assets.bedroom, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), Assets.bedroom.getWidth() * 2,
					Assets.bedroom.getHeight() * 2, null);

			for (Entity e : handler.getGame().geteManager().getEntities())
				handler.getGame().geteManager().render(e, g);

			g.drawImage(Assets.bedroomTop, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), Assets.bedroom.getWidth() * 2,
					Assets.bedroom.getHeight() * 2, null);
			...
			currentWidth = currentLocation.getCurrentFrame().getWidth() * 2;
			currentHeight = currentLocation.getCurrentFrame().getHeight() * 2;
			g.drawImage(currentLocation.getCurrentFrame(), (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), currentLocation.getCurrentFrame().getWidth() * 2,
					currentLocation.getCurrentFrame().getHeight() * 2, null);

			if (activeShopKeeper)
				g.drawImage(Assets.zaria, (int) (1064 - handler.getCamera().getxOffset()),
						(int) (1560 - handler.getCamera().getyOffset()), Assets.zaria.getWidth() * 2,
						Assets.zaria.getHeight() * 2, null);
			if (activebSmith)
				g.drawImage(Assets.max, (int) (1212 - handler.getCamera().getxOffset()),
						(int) (1563 - handler.getCamera().getyOffset()), Assets.max.getWidth() * 2,
						Assets.max.getHeight() * 2, null);

			g.drawImage(Assets.buildingDecor, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), currentLocation.getCurrentFrame().getWidth() * 2,
					currentLocation.getCurrentFrame().getHeight() * 2, null);
			...
			g.drawImage(Assets.buildings, (int) -handler.getCamera().getxOffset(),
					(int) -handler.getCamera().getyOffset(), currentLocation.getCurrentFrame().getWidth() * 2,
					currentLocation.getCurrentFrame().getHeight() * 2, null);
			...
			if (state == "QUESTSELECT")
				...
				bSmith.render(g);
			if (!activeBoat)
				g.drawImage(Assets.boat, (int) (940 - handler.getCamera().getxOffset()),
						(int) (2800 - handler.getCamera().getyOffset()), Assets.boat.getWidth() * 2,
						Assets.boat.getHeight() * 2, null);
			else {
				boatX += 6;
				g.drawImage(Assets.boatWPlayer, (int) (940 + boatX - handler.getCamera().getxOffset()),
						(int) (2800 - handler.getCamera().getyOffset()), Assets.boat.getWidth() * 2,
						Assets.boat.getHeight() * 2, null);
		...
		for (Event e : events) {
			if (handler.getGame().getPlayer().getCollisionBounds(0, 0).intersects(e.getEventSpace())
					&& e.meetsRequirements() && !e.isRunning())
				e.setRunning(true);
			if (e.isRunning()) {
				isEventRunning = true;
				e.run(g);
