...
public class MenuState extends State {
	...
	private enum State {
		TITLE_SCREEN, LOAD_SELECTION, START_GAME, GAME_STATE
	}

	private enum Fading {
		FADING_IN, FADING_OUT, STABLE
	}
	...
	private void fadeIntoState(State toState) {
		if (toState == State.LOAD_SELECTION)
			...
			selectorBox.setAll(
					new Rectangle(handler.getGame().getWidth() - 60, handler.getGame().getHeight() - 60, 45, 45));
	...
	public void render(Graphics g) {
		if (state == State.TITLE_SCREEN) {
			...
			handler.getGame();
			// Utils.drawFadedImage(g, alpha, Assets.selector700x150, handler.getWidth()/2 -
			// 350, handler.getHeight()/8 + selector * 150, 700, 150);
			for (int i = 0; i < Game.SAVE_FILE_COUNT; ++i)
				g.drawImage(Assets.box700x150, handler.getWidth() / 2 - 340 - 10 * (selector == i ? 1 : 0),
						handler.getHeight() / 8 + (150 * i) + 10 - 10 * (selector == i ? 1 : 0),
						680 + 20 * (selector == i ? 1 : 0), 130 + 20 * (selector == i ? 1 : 0), null);

			for (int i = 0; i < Assets.menuNumbers.length; ++i)
				g.drawImage(Assets.menuNumbers[i], handler.getWidth() / 2 - 330, handler.getHeight() / 8 + 15 + 150 * i,
						null);
			...
			for (int i = 0; i < Game.SAVE_FILE_COUNT; i++) {
				if (handler.getGame().getUserData(i).isFile()) {
					g.setFont(new Font(Game.font, Font.BOLD, 30));
					Effects.drawStringWithShadow(g, "FILE " + Integer.toString(i + 1), Color.WHITE, 300,
							handler.getHeight() / 8 + 70 + i * 150);
					...
					for (int a = 0; a < handler.getGame().getUserData(i).getTotalHeartCrystals(); a++) {
						if (a < handler.getGame().getUserData(i).getActiveHeartCrystals())
							g.drawImage(Assets.heartCrystal, 620 + a * 17, handler.getHeight() / 8 + 47 + i * 150,
									null);
						else
							g.drawImage(Assets.usedHeartCrystal, 620 + a * 17, handler.getHeight() / 8 + 47 + i * 150,
									null);
					}

					g.drawLine(300, handler.getHeight() / 8 + 75 + i * 150, 800,
							handler.getHeight() / 8 + 75 + i * 150);

					g.setFont(new Font(Game.font, Font.BOLD, 15));
					Effects.drawStringWithShadow(g,
							String.format("Total play time %34s", handler.getGame().getUserData(i).getTotalPlayTime()),
							new Color(134, 211, 61), 370, handler.getHeight() / 8 + 90 + i * 150);
				} else {
					text = "NEW FILE";
					g.setFont(new Font(Game.font, Font.BOLD, 30));
					Effects.drawStringWithShadow(g, text, Color.WHITE,
							(handler.getWidth() - g.getFontMetrics().stringWidth(text)) / 2 - 2,
							handler.getHeight() / 8 + 95 + i * 150);
				}
			}
			...
			g.drawImage(Assets.box400x100, handler.getGame().getWidth() / 2 - 440 - 10 * (selector == 0 ? 1 : 0),
					410 - 10 * (selector == 0 ? 1 : 0), 380 + 20 * (selector == 0 ? 1 : 0),
					80 + 20 * (selector == 0 ? 1 : 0), null);
			g.drawImage(Assets.box400x100, handler.getGame().getWidth() / 2 - 40 - 10 * (selector == 1 ? 1 : 0),
					410 - 10 * (selector == 1 ? 1 : 0), 380 + 20 * (selector == 1 ? 1 : 0),
					80 + 20 * (selector == 1 ? 1 : 0), null);
			g.drawImage(Assets.box100x100, handler.getGame().getWidth() / 2 + 360 - 10 * (selector == 2 ? 1 : 0),
					410 - 10 * (selector == 2 ? 1 : 0), 80 + 20 * (selector == 2 ? 1 : 0),
					80 + 20 * (selector == 2 ? 1 : 0), null);
			...
			if (handler.getGame().getUserData(fileNum).isFile()) {
				...
				Effects.drawStringWithShadow(g, "FILE " + Integer.toString(fileNum + 1), Color.WHITE, 300,
						handler.getHeight() / 8 + 70);
				...
				Effects.drawStringWithShadow(g,
						String.format("Total play time %34s",
								handler.getGame().getUserData(fileNum).getTotalPlayTime()),
						new Color(134, 211, 61), 370, handler.getHeight() / 8 + 90);
			} else {
				text = "NEW FILE";
				g.setFont(new Font(handler.getGame().font, Font.BOLD, 30));
				Effects.drawStringWithShadow(g, text, Color.WHITE,
						(handler.getWidth() - g.getFontMetrics().stringWidth(text)) / 2 - 2,
						handler.getHeight() / 8 + 95);
			}
