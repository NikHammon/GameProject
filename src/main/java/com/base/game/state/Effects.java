	...
	public void render(Graphics g) {
		...
		if (renderSmokeScreen) {
			if (smoke.isComplete()) {
				...
				g.drawImage(smoke.getCurrentFrame(),
						(int) ((10 * Tile.TILE_WIDTH - 32) - handler.getCamera().getxOffset()),
						(int) (12 * Tile.TILE_HEIGHT - 50 - handler.getCamera().getyOffset()),
						smoke.getCurrentFrame().getWidth() * 2, smoke.getCurrentFrame().getHeight() * 2, null);
				g.drawImage(smoke.getCurrentFrame(),
						(int) ((13.5f * Tile.TILE_WIDTH - 32) - handler.getCamera().getxOffset()),
						(int) (12 * Tile.TILE_HEIGHT - 50 - handler.getCamera().getyOffset()),
						smoke.getCurrentFrame().getWidth() * 2, smoke.getCurrentFrame().getHeight() * 2, null);
			}
		}
		...
		if (thunderStorm) {
			if (tStormState == "INIT") {
				...
				if (darkAlpha >= .3) {
					tStormState = "STORM";
					flashColor = ThreadLocalRandom.current().nextInt(400);
					darkAlpha = .3f;
				}
			} else if (tStormState == "STORM") {
				if (flashColor == 0)
					Utils.drawFadedImage(g2d, thunderAlpha, Assets.yellowLightning, 0, 0, handler.getWidth(),
							handler.getHeight());
				else if (flashColor == 1)
					Utils.drawFadedImage(g2d, thunderAlpha, Assets.blueLightning, 0, 0, handler.getWidth(),
							handler.getHeight());
				else if (flashColor == 2)
					Utils.drawFadedImage(g2d, thunderAlpha, Assets.whiteOut, 0, 0, handler.getWidth(),
							handler.getHeight());
				...
				if (lightningStorm) {
					if (!boltActive && ThreadLocalRandom.current().nextInt(100) == 0) {
						boltActive = true;
						boltX = (ThreadLocalRandom.current().nextInt(2) == 1)
								? ThreadLocalRandom.current().nextInt(handler.getWidth() / 2 - 72)
								: ThreadLocalRandom.current().nextInt(handler.getWidth() - 72) + handler.getWidth() / 2
										+ 112;
						boltY = -ThreadLocalRandom.current().nextInt(100);
					}
					if (boltActive) {
						if (lightning.isComplete()) {
							...
							lightning.update();
							g.drawImage(lightning.getCurrentFrame(), boltX, boltY,
									lightning.getCurrentFrame().getWidth() * 2,
									lightning.getCurrentFrame().getHeight() * 2, null);
						}
					}
				}
			}
		...
		if (drawRect) {
			...
			g2d.fillRect((int) (x - handler.getCamera().getxOffset()), (int) (y - handler.getCamera().getyOffset()),
					width, height);
		}
		...
		switch (state) {
			...
			case "DUNGEONDISPLAY":
				...
				g.drawImage(handler.getGame().getPlayer().getCurrentDungeon().getSymbol(), handler.getWidth() / 2 - 48,
						280, 96, 96, null);
