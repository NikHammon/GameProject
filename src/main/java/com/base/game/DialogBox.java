...
public class DialogBox
{
	...
	public void setScriptNum(int scriptNum)
	{
		...
		if(scriptNum == 3 && !handler.getGame().getGameState().getLevelManager().getBaseCamp().getEvents().get(0).isRunning())// wht the helllllll
		{
	...
	public void render(Graphics g) 
	{
		...
		{
			if(!questionBox)
				g.drawImage(Assets.dialogArrow, handler.getGame().getWidth()/2 - 12, (int)(handler.getGame().getHeight() - 30 + arrowPos), 24, 24, null);
			else
			{
				//g.drawImage(Assets.selector200x100, handler.getWidth()-205, handler.getGame().getHeight() - 405 + (selector? 0:100), null);
	...
	public boolean isNextAvailible() {return nextAvailable;}
	public void setNextAvailible(boolean nextAvailible) {this.nextAvailable = nextAvailible;}		
